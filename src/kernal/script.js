...
function init() {
	...
	for (var i = 0; i < 11; i++) {
		var key = "com.pagetable.kernal.column_" + i;
	...
	var key = "com.pagetable.kernal.column_decimal";
...
function toggleDecimal(visible) {
	...
	for (var i = 0; i < tbl.rows.length; i++) {
		tbl.rows[i].cells[3].style.display = visible ? "" : "none";
	}
	var key = "com.pagetable.kernal.column_decimal";
...
function hideCol(col, checked) {
	var tbl = document.getElementById("disassembly_table");
	for (var i = 0; i < tbl.rows.length; i++) {
		tbl.rows[i].cells[col+4].style.display = checked ? "" : "none";
	}
	var key = "com.pagetable.kernal.column_" + col;
	if (checked) {
		localStorage.removeItem(key);
	} else {
